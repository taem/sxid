...
static char *fill_space(int count)
{
	char *p;
	int i;
...
static char *format_ugid(uid_t uid, gid_t gid, mode_t mode)
{
...
static char *super_format_ugid(struct file_entry *p)
{
	struct passwd *pw;
	struct group *gr;
	char *ret_val, *temp;
...
static void check_add_rem(FILE * mail_file)
{
...
static void check_u_g(FILE * mail_file)
{
	struct file_entry *p = global_log;
...
static void check_forbidden(FILE * mail_file)
{
	struct file_entry *p = global_log;
	char init;
...
static void do_list_all(FILE * mail_file)
{
	struct file_entry *p = global_log;
...
void save_and_rotate(void)
{
	...
	if (new_log == NULL) {
		fprintf(stdout, "W: Could not open new log file %s, not saving "
